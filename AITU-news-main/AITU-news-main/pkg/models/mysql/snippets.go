...
func (m *SnippetModel) Insert(title, content, expires string) (int, error) {
	// Write the SQL statement we want to execute. I've split it over two lines // for readability (which is why it's surrounded with backquotes instead
...
func (m *SnippetModel) Get(id int) (*models.News, error) {
...
func (m *SnippetModel) Latest() ([]*models.News, error) {
	// Write the SQL statement we want to execute.
	stmt := `SELECT id, title, content, created, expires FROM news WHERE expires > UTC_TIMESTAMP() ORDER BY created DESC LIMIT 10`
	...
	for rows.Next() {
		...
		s := &models.News{}
		// Use rows.Scan() to copy the values from each field in the row to the // new News object that we created. Again, the arguments to row.Scan() // must be pointers to the place you want to copy the data into, and the // number of arguments must be exactly the same as the number of
